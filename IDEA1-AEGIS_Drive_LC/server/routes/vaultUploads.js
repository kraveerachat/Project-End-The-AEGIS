...
/** คีย์ที่ create ของครอบครัว tree ยอมรับ — ชุดเดียวกับที่ route เก่าอ่านอยู่แล้ว (ไม่มีชื่อ/parent/node/path โดยโครงสร้าง) */
const CREATE_KEYS = ['formatVersion', 'contentIdB64', 'ciphertextSize', 'chunkSize', 'chunkCount', 'wrappedDekB64', 'wrapIvB64', 'metaIvB64', 'metaB64']
const strictKeys = (body, allowed) => body !== null && typeof body === 'object' && !Array.isArray(body) && Object.keys(body).every((k) => allowed.includes(k))
// ⚠️ ไม่ echo ชื่อฟิลด์ที่ถูกปฏิเสธ: ค่านั้นอาจเป็นชื่อไฟล์/path ที่ client ไม่ควรส่งมาตั้งแต่แรก
const unknownField = (res) => res.status(400).json({ error: 'Unknown field', code: 'UNKNOWN_FIELD' })

/**
 * PR #157 Task 4.1: handler ทั้งหกของครอบครัว V2 upload สร้างจากโรงงานเดียว
 *   mode 'legacy' → /api/vault/uploads/*      พฤติกรรมเดิมทุกไบต์ (fence: FLAT เท่านั้น)
 *   mode 'tree'   → /api/vault/tree/uploads/* เส้นทางเดียวกัน, ไบต์เดียวกัน, chunk/concurrency/retry เดิม —
 *                   ต่างเพียง (ก) fence: TREE_V1 เท่านั้น (ข) strict body (ค) commit บันทึก lifecycle UNREFERENCED
 *                   ใน transaction เดียวกับแถว blob และตอบ lifecycle เพิ่มหนึ่งฟิลด์
 * ⚠️ ไม่มี mode ใดรับชื่อไฟล์/parent/node/path/MIME — ครอบครัว tree ไม่ "เพิ่ม" ฟิลด์ใด มันแค่ "ปฏิเสธ" ฟิลด์แปลกปลอม
...
export function createVaultUploadHandlers({ mode }) {
  ...
  const limits = async (req, res, next) => {
    try {
      res.json({
        formatVersion: 2,
  ...
  const chunk = async (req, res, next) => {
    try {
      const session = await loadOwnSession(req, res)
      if (!session) return undefined
  ...
  return { mutationFence, limits, create, status, chunk, commit, cancel }
}

/** ผูก handler ชุดหนึ่งเข้ากับ router — ลำดับ/เส้นทาง/ด่านเหมือนกันทั้งสองครอบครัว */
export function mountVaultUploadHandlers(router, h) {
  router.get('/limits', requireAuth, h.limits)
  router.post('/', requireAuth, h.mutationFence, h.create)
  router.get('/:uploadId', requireAuth, h.status)
  router.put('/:uploadId/chunks/:index', requireAuth, h.mutationFence, h.chunk)
  router.post('/:uploadId/commit', requireAuth, h.mutationFence, h.commit)
  router.delete('/:uploadId', requireAuth, h.cancel)
  return router
}

export const vaultUploadsRouter = mountVaultUploadHandlers(Router({ mergeParams: true }), createVaultUploadHandlers({ mode: 'legacy' }))
