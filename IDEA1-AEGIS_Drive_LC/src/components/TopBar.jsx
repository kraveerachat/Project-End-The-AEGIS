...
export function TopBar({ t, lang = 'en', scrolled, user, health, onProfile, onSettings, onSignOut, openMobileNav, resolvedTheme = 'light', onThemeChange }) {
  ...
  const locale = lang === 'th' ? 'th-TH-u-ca-gregory' : lang === 'zh' ? 'zh-CN' : 'en-GB'
  const clockText = now.toLocaleTimeString(locale, { hour12: false })
  const dateText = now.toLocaleDateString(locale, { day: '2-digit', month: 'short', year: 'numeric' })
