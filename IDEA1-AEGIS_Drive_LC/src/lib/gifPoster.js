...
/**
 * เพดานของ "ทรัพยากรที่ยอมดึงมาถอด poster ตอน idle" — ไม่ใช่เพดานอัปโหลด/ขนาดไฟล์ของระบบ
 * (ดู server/config/transferLimits.js ซึ่งไม่รู้จักค่านี้และต้องไม่รู้จัก)
 *
 * ⚠️ ไฟล์ GIF ที่ใช้ยอมรับจริงบน Production มีขนาด ~49.7 MB — เพดานนี้ต้องครอบมัน ไม่งั้นการ์ด
 *    ตกไปเป็นไอคอนเพียงเพราะขนาด ซึ่งคือสิ่งที่ Round 10 ถูกขอให้แก้ ตั้งไว้ที่ 96 MiB:
 *    ครอบไฟล์จริงพร้อมช่องว่าง แต่ยังกันไม่ให้การ์ดใบเดียวดึงหลายร้อย MB ตอน idle
 *    ไฟล์ที่ใหญ่กว่านี้ยังชี้แล้วเล่นได้ตามปกติ — เพดานนี้ตัดสินแค่ poster ตอน idle
 */
export const GIF_POSTER_MAX_BYTES = 96 * 1024 * 1024
