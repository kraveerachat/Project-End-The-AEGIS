...
test('PS5-T02-1 unknown, malformed, revoked and trashed links are one indistinguishable refusal', async () => {
  ...
  // ⚠️ The per-response CSP nonce is random BY DESIGN, so it is normalised out
  ...
  for (const [label, token] of Object.entries(cases)) {
    ...
    // The nonce must actually differ per response — normalising it away must
    // not quietly hide a fixed nonce.
    assert.match(body, /nonce="[A-Za-z0-9+/=]{16,}"/, `${label} must carry a per-response CSP nonce`)
