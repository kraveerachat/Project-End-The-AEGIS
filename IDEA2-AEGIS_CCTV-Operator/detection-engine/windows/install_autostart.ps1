...
$identityWasCopied = -not [string]::IsNullOrWhiteSpace($resolvedIdentitySource)
...
if (-not [string]::IsNullOrWhiteSpace($resolvedConfiguration)) {
    Copy-Item -LiteralPath $resolvedConfiguration -Destination $runtimeEnv -Force
}
...
if (-not [string]::IsNullOrWhiteSpace($resolvedIdentitySource)) {
    Copy-Item -LiteralPath $resolvedIdentitySource -Destination $runtimeIdentity -Force
}
elseif (-not (Test-Path -LiteralPath $runtimeIdentity -PathType Leaf)) {
...
if (-not [string]::IsNullOrWhiteSpace($resolvedKnownHostsSource)) {
    Copy-Item -LiteralPath $resolvedKnownHostsSource -Destination $runtimeKnownHosts -Force
}
...
# OpenSSH running as SYSTEM rejects broadly readable private keys. Apply the
# ACL only when this run copied a fresh runtime key. A repair reuses the
# already SYSTEM-owned key without trying to read or take ownership of it.
if ($identityWasCopied) {
