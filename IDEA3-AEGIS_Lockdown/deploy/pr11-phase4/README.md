...
- File staging & permissions: stages `/etc/aegis-idea3/credentials/` (directory `root:aegis-idea3` mode 0750; secret files mode 0600, `restore.credential` owned by `aegis-idea3` because the Core reads and ownership-checks it directly), `/etc/aegis-idea3/core.env` (mode 0600), `/etc/systemd/system/aegis-idea3-core.service` (mode 0644), and immutable release pointer `/opt/aegis-idea3/current` symlink, created only after the release (`venv/bin/python`, `aegis_soc/supervisor.py`, root-owned, not group/world-writable) is proven to exist; L7 never builds the release.
