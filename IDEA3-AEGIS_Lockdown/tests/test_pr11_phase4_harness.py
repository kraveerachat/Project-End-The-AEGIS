...
def test_only_reviewed_stage_handlers_are_registered() -> None:
    ...
    assert {p.name for p in stages.iterdir() if p.is_dir()} == {"L2", "L3", "L4", "L6b"}
    for name in ("L2", "L3", "L4", "L6b"):
...
def test_gate_live_mode_for_mutating_stage_fails_without_registered_handler(tmp_path: Path) -> None:
    result = gate(tmp_path, "--stage", "L5", "--mode", "live", auth=auth_record("L5"), k3=k3_record("L5"))
