...
export function MetricCard({ icon: Icon, label, value, suffix, status, statusLabel, statusAriaLabel, trend, detail }) {
  const TrendIcon = trend?.direction === 'down' ? ArrowDownRight : ArrowUpRight
  return (
    <article className="metric-card">
      <div className="metric-card__top">
        {Icon && <span className="icon-box"><Icon size={19} aria-hidden="true" /></span>}
        {status && <StatusBadge status={status} compact label={statusLabel} ariaLabel={statusAriaLabel} />}
      </div>
