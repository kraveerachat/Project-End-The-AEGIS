...
> **2026-08-06**: **Infrastructure (as-built) track added to the vault.** A new set of notes under `00-MOC/`, `10-Network/`, `20-Server/`, `30-RemoteAccess/`, `40-Deployment/`, and `90-Status/` records what physically exists on the hardware, hubbed at [[00-MOC/AEGIS-Infrastructure-MOC]]. Network segmentation (VLAN 10/20/30 on MikroTik RB750r2 + TL-SG105E) and remote access ([[30-RemoteAccess/Twingate-Setup|Twingate ZTNA]], SSH from outside the network) are **working and tested**; The complete AEGIS source and vault are also published safely on GitHub branch `aegis-system` (the target repository `main` remains a separate Sina app and was not overwritten); server hardening and application deployment are **not** finished. ⚠️ **Scope clarification for the entries above**: every "Docker stack healthy / `http://localhost/monitor/` HTTP 200" result recorded in this note was measured on the **developer machine** — the Beelink server `aegis-system` (`192.168.10.10`) currently runs **only the Twingate Connector**, and the Gateway/Drive/Monitor/PostgreSQL stack has **never been deployed there**. See [[40-Deployment/Docker-Stack-Plan]] and item 7 of [[90-Status/Document-Conflicts]]. Open infrastructure work is tracked in [[90-Status/Open-Items-Backlog]] (P1: verify real UFW state, close SSH hardening, rotate the Twingate connector token).
...
| **UFW production rules on the Beelink server** | 🔴 Open · **state unverified** | The host firewall was disabled temporarily while inter-VLAN routing was being tested and was never confirmed re-enabled. Do **not** assume it is on — run `ufw status verbose` first. [[20-Server/Beelink-Ubuntu-Host]] |
| **SSH hardening not closed (infrastructure)** | 🔴 Open | `PasswordAuthentication` is still `yes`. Only `admin-main` has a working key; the other two accounts must generate keys **on their own machines** (an earlier attempt wrongly generated them on the admin laptop) and the duplicate public keys must be removed. Ordered 8-step checklist in [[20-Server/SSH-Hardening-Status]]. |
| **Twingate connector token rotation** | 🔴 Open | The connector token was displayed on screen during setup and must be treated as leaked. The connector also has no restart policy or health check — if the container dies, the whole team loses remote access to a headless machine. [[30-RemoteAccess/Twingate-Setup]] |
| **Production stack never deployed to the Beelink** | 🔴 Open | Gateway/Drive/Monitor/PostgreSQL run on the dev machine only. Deployment is additionally blocked on an undecided design question: Macvlan `.11`/`.12` (per the report) may be unreachable from the bridge-networked Twingate connector due to Linux macvlan-to-host isolation. [[40-Deployment/Docker-Stack-Plan]] |
| **Report ↔ reality: remote access path** | 🟠 Reconciliation Needed | Report §2.3.4/§3.5.6 describe OpenVPN + Twingate in parallel and require entering VLAN 30 before reaching other services. Reality: Twingate only, one SSH resource, pointing straight at VLAN 10. OpenVPN never worked (double NAT). Undecided whether to change the resource or the report. [[90-Status/Document-Conflicts]] |
