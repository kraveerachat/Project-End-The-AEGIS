...
- Desktop 1280×720 — **interactively exercised during the PR #95 reconciliation** on the reconciled build: the telemetry card renders row 1 `CPU | RAM | Disk` and row 2 `Network | Uptime | CPU temperature`, confirmed from each tile's actual bounding rectangle rather than markup order, with no Twingate tile and no horizontal overflow. Mobile 435×982 re-checked the same six tiles with `scrollWidth == clientWidth == 435`. Widths 1440/~900/~768 remain checked by source/tests only.
