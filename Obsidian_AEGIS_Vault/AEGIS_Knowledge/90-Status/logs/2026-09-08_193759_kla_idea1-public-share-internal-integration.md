...
## Stage B attempt #4 — EXECUTED — PASS (appended 2026-09-08)

⚠️ **Attempts #1, #2 and #3 above are preserved exactly as historical evidence.
Nothing in them is edited, softened or removed by this result.**

**Accepted source SHA: `15c43d6bc4d5f3d7287d20e855677a44f0ec0231`.**

Run on the real Beelink host `aegis-system`, project
`aegis-ps6-stage-b-20260908-185817-2053260`, through
`sudo -n env -u DOCKER_HOST docker`.

| Fact | Result |
| :--- | :--- |
| Source SHA | **`15c43d6bc4d5f3d7287d20e855677a44f0ec0231`** |
| Guards 0–10 | all passed, including guard 9 (classify before parsing) and guard 10 (streamed upload with backpressure) |
| Tests | **16** |
| Passed | **16** |
| Failed | **0** |
| Skipped | **0** |
...
| `STAGE_B_RUNNER_RC` | **0** |
| Duration | 149.0 s for the matrix |

### PS6-INT-4 — the decisive evidence

The 64 MiB private-path upload that failed as `EPIPE` in attempt #3, on the same
shipped endpoint, at the same size, with the same deterministic bytes:

| Counter | Value |
| :--- | ---: |
| deterministic file bytes | **67,108,864** (64 MiB) |
| request `Content-Length` | **67,109,045** |
| upload chunk size | **262,144** bytes |
| generated bytes | **67,108,864** |
| written bytes | **67,109,045** |
| **drain waits** | **256** |
| `endedRequest` | **true** |
| `responseStatus` | **201** |

`generatedBytes` equals the acceptance size exactly, `writtenBytes` equals the
declared `Content-Length` exactly, and the request was ended only after every
chunk had been accepted. **256 drain waits** is the backpressure the previous
client had no way to honour: the socket pushed back 256 times across the
transfer, and the streamed client waited each time instead of continuing to
write into a queue nobody was draining.

⚠️ **This confirms the streaming/backpressure-aware harness completed the same
64 MiB acceptance object successfully.** It does not retroactively prove the
single unbounded `req.write()` was the sole cause of the attempt #3 `EPIPE` — the
counters show the streamed client succeeds where the one-shot client failed, on
the same host, the same endpoint and the same payload, which is the strongest
statement the evidence supports.

### All sixteen subtests passed

| Subtest | What it proved | ms |
| :--- | :--- | ---: |
| PS6-INT-1 | three internal isolated networks, no host port anywhere | 317 |
| PS6-INT-2 | the recipient reaches the gateway and nothing else | 5,176 |
| PS6-INT-3 | migration 009 load-bearing on a real 008-era disposable PostgreSQL | 1,850 |
| PS6-INT-4 | 64 MiB private upload + public share creation | 8,172 |
| PS6-INT-5 | normal redemption end to end through the real gateway | 1,774 |
| PS6-INT-6 | a 75-second slow-client stall completes intact | 75,842 |
| PS6-INT-7 | an interrupted transfer harms neither tier | 2,688 |
| PS6-INT-8 | four concurrent downloads all complete intact | 2,922 |
| PS6-INT-9 | gateway public surface / default deny | 690 |
| PS6-INT-10 | forged forwarding headers cannot move source attribution | 993 |
| PS6-INT-11 | an unknown Host terminates at the gateway | 806 |
| PS6-INT-12 | the ingress split holds in both directions | 670 |
| PS6-INT-13 | B5 — the gateway reaches only its upstream | 6,507 |
| PS6-INT-14 | revocation propagates immediately | 898 |
| PS6-INT-15 | harness teardown | 11,180 |
| **PS6-INT** (parent) | **all fifteen, in order** | **148,749** |

Every subtest that attempts #2 and #3 could only report as blocked or cascading
has now **executed and passed on server hardware**.

### Production safety — unchanged, and verified after cleanup

- Production pre/post inventory **IDENTICAL** — every container ID, image ID,
  Compose project, running state, health state and network attachment, plus every
  network, volume and image outside the project.
- Every Production service remained **healthy**: `aegis-prod-drive-1`,
  `aegis-prod-hub-1`, `aegis-prod-monitor-1`, `aegis-prod-postgres-1`,
  `twingate-aegis-connector-02`.
- `aegis_postgres_data` **present**; `aegis_drive_storage` **present**.
- No PS6 container, network, volume or image survived.
- Both PS6-built image IDs removed and verified gone **by ID**:
  `sha256:6b163e02…` (drive) and `sha256:66b56e0b…` (public-share-gateway).
- `postgres:15-alpine`, `node:20-alpine` and `nginx:alpine` **preserved**.
- **All 50 `aegis-prod` rows unchanged.**
- The Compose env file removed:
  `/tmp/aegis-ps6-stage-b-20260908-185817-2053260/evidence/compose.env`.
- The PS6 temporary workdir removed; **nothing matches `/tmp/aegis-ps6-*` or
  `/tmp/ps6-stage-b*`**.
- Production migration 009 was **NOT** read or applied; the Production Public UI
  flag was **NOT** read or changed; **no Production PostgreSQL connection**; no
  Production config or runtime mutation.
- No DNS/TLS/NAT/tunnel/MikroTik/UFW/VLAN/Twingate change. **No prune.**
- **PUBLIC-SHARE-7 not started.**

## PUBLIC-SHARE-6 internal integration acceptance = PASS

The real PUBLIC-SHARE-3 gateway image, in front of the real AEGIS Drive image, on
a real PostgreSQL 15 carrying the real schema and the real migration 009, across
three internal isolated Docker networks, on AEGIS server hardware — sixteen of
sixteen, exit 0, zero post-run check failures, runner RC 0, Production identical
and healthy throughout.

⚠️ **What that sentence does and does not mean.**

- **This is internal isolated acceptance only.** The recipient is a container on
  an isolated Docker network with no default route — not someone on ordinary
  Internet access.
- **No public Internet ingress exists.** No port is published, no DNS record, no
  TLS certificate, no NAT rule, no tunnel and no firewall change. Nothing is
  reachable from outside the host.
- **G4 remains OPEN** — no ingress method has been chosen.
- **G5 remains OPEN** — no exposure has been authorised.
- **G6 remains OPEN** — completion has not been granted.
- **Public Internet Share remains NOT IMPLEMENTED.** Production gateway = NO,
  ...
  DNS/TLS/NAT/tunnel/ingress = NO, external acceptance = NO.
- **PUBLIC-SHARE-7 has NOT started.**

This closeout is **documentation only**. The accepted runtime evidence belongs to
SHA `15c43d6bc4d5f3d7287d20e855677a44f0ec0231`, and no source, test, harness,
runner, Compose, schema, migration, Dockerfile or configuration file was changed
by it — so nothing here invalidates the tree that was actually run.

### Closeout verification (non-runtime only)

- `git fetch origin`; `git rev-parse origin/main` and
  `git ls-remote origin refs/heads/main` both report
  **`c68946cbe917a71349a8234a4bc028fbf4c6967d`** — **unchanged** since the merge
  already in this branch. **No second merge was created**, so the reviewed and
  accepted tree is not disturbed.
- `node --test tests/collaborationPolicy.test.mjs` — **18 tests, 18 passed, 0
  failed.**
- `node scripts/validate-vault.mjs --vault Obsidian_AEGIS_Vault/AEGIS_Knowledge`
  — passed, with the same two pre-existing owner-review canvas warnings.
- `git diff --check`, `git diff --cached --check`, `git status --short` — clean.
- `git diff --name-only 15c43d6b..HEAD` — three Markdown files, all under
  `Obsidian_AEGIS_Vault/AEGIS_Knowledge/`: this receipt,
  `idea1/idea1-status.md` and `idea1/idea1-public-share-architecture.md`.
  **Documentation only**, verified by path and by extension.
- **Docker Stage B was NOT re-run.**
...
- ⚠️ **This bullet is superseded and kept for the record.** It read: *"This is
  Stage A only, and Stage B is gate-approved but unexecuted … `run-stage-b.sh` is
  written and syntax-checked but has never been executed anywhere."* That was
  true when written. **Stage B has since been executed four times on the AEGIS
  server host — attempts #1, #2 and #3 failed safely, and attempt #4 PASSED
  16/16 against SHA `15c43d6b…`.** The runner now carries real runtime evidence.
  What has *not* changed: no Production service restart, no Production data
  mount, no host port and no ingress change was ever attempted or claimed.
- **Stage B acceptance is internal and isolated, not Internet acceptance.** The
  recipient is a container on an isolated Docker network with no default route.
  No port is published, no DNS record, TLS certificate, NAT rule, tunnel or
  firewall change exists. **G4, G5 and G6 remain open**, and PUBLIC-SHARE-7 has
  not started.
- **The attempt #3 `EPIPE` root cause is not claimed.** The streamed,
  backpressure-aware client succeeds where the one-shot client failed, on the
  same host, endpoint and payload, with 256 drain waits recorded. That is what
  the evidence supports; it is not a proof that the unbounded write was the sole
  cause.
