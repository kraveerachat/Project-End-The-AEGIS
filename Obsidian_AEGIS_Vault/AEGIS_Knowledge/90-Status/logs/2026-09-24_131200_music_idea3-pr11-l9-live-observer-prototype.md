...
- New `deploy/pr11-phase4/p4-l9-live-observe.py`: opens the running Core's protocol and audit SQLite stores read-only and passes only with fresh accepted STATUS rows for the device, zero command rows, zero allocated sequence and zero CUT/RESTORE audit events; output is counts and stable codes only.
...
- **Not wired**: `apply.sh`/`p4-l9-auth.py` refusals are unchanged; 16 fixture tests use real `ProtocolStore` databases.
...
- `pytest tests/test_pr11_phase4_l9_live_observer.py tests/test_pr11_phase4_l9_handler.py` — pass: 170 passed
- `pytest tests/test_pr11_phase4_*.py tests/test_pr11_k10_server_ca.py` — pass: 876 passed
- `pytest tests` (IDEA3 full) — pass: 1927 passed, 6 skipped
