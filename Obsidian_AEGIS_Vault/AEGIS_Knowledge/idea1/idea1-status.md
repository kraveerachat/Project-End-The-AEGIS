...
- Branch: `feat/idea1-vault-files-ux-navigation`; PR: #171 (Draft) at the Human Browser Acceptance checkpoint.
- Base: originally `origin/main` at `15ccee1597e31529f266bee822125393f01a5e23` (merged PR #157); reconciled by merge with current `origin/main` at `c88cd04b3c171be72701982373f9062797b27cc8` on 2026-09-22.
- State: **V9 SELECTION UX AUTOMATED PASS / HUMAN OWNER RETEST PENDING (2026-09-23)** — Human Owner accepted V8 video posters without hover for old and newly uploaded videos, with hover/image/GIF behavior normal. V9 makes the unlocked Vault screen root the single marquee geometry/input/overlay surface, preserves interactive/text/card ignore boundaries, and clears non-empty selection only when a valid non-modifier background gesture resolves below the drag threshold. Focused verification is 112/112 PASS; build, governance, vault validation, and diff checks pass. The existing isolated PostgreSQL 15 / TREE_V1 runtime is reused, Production remains untouched, and PR #171 stays Draft with no final receipt.
