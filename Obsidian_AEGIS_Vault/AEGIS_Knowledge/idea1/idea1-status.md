...
- State: **V7 LOCAL ENGINEERING PASS / HUMAN OWNER VIDEO RETEST PENDING (2026-09-23)** — a newly uploaded Vault video now receives its static poster automatically without hover. The scheduler remains stable across independent manifest/blob-inventory refreshes, preserves already-ready posters, and performs one bounded retry when a `BLOB_NOT_READY` job crosses to eligible; hover remains motion-only and mouse leave restores the existing poster. The required focused suite passes 64/64 with zero failures or skips; production build, governance, vault validation, and diff checks pass. No visual markup or styles changed, so the conditional Impeccable rerun is not applicable. The existing isolated PostgreSQL 15 / TREE_V1 runtime is reused and Production remains untouched. Live engineering-browser qualification is authentication-gated because the disposable account password was not available; credentials were not guessed or reset. PR #171 stays Draft with no final receipt until Human Owner browser retest.
