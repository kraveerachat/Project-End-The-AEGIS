...
- **Dashboard:** the locally verified source order is `CPU | RAM | Disk` then `Network | Uptime | Temperature`. Temperature is Server / CPU Package Temperature from an exact `x86_pkg_temp` Linux sysfs sensor discovered independently of thermal-zone numbering and projected through the host telemetry contract. Missing/unreadable/malformed/negative readings are unavailable; there is no SSD fallback, browser threshold, hardcoded reading, or synthesized zero, while a measured zero remains valid evidence. The old Dashboard Twingate placeholder is removed.
- **Storage:** Disk Health now presents `Model | Device | SMART` then `Twingate Local Connector | Power-on Hours | Device Capacity`. SMART SSD temperature remains unchanged backend disk-health evidence for physical-disk health and `temperature-high` warnings, but is not shown in the fact grid and is not used as Dashboard Server Temperature. Connector state uses only `/api/remote-access.localConnector`; Twingate control-plane state remains NOT MEASURED. Connector evidence stays visible even when disk-health evidence is unavailable.
...
Local verification for this pass, including the CPU-package correction: focused IDEA1 regressions **192 total / 189 pass / 3 PostgreSQL-only skips / 0 fail**; host-agent suite **144 total / 141 pass / 3 POSIX-only skips / 0 fail**; full IDEA1 suite **1,046 total / 978 pass / 67 PostgreSQL-only skips / 1 pre-existing AUTOLOCK-5 false-positive failure**; production build PASS with the existing >500 kB chunk warning. Production telemetry-chain verification and visual acceptance remain pending.
