...
| VUX-S5 | Human-acceptance V4 corrections for Vault folder cards and multi-select drag pipeline | LOCAL PASS / OWNER RETEST PENDING | Root causes confirmed: `VaultFolderTile` rendered `childCount` natively unlike normal Files (violating the parity rule); the drag pipeline relied on a potentially stale React closure inside `VaultTreeScreen` causing N items to collapse to 1 during the drop action. Corrected `VaultFolderTile` to remove `childCount` and rely strictly on folder icon and name. Corrected `useVaultTree` to capture a synchronous `dragSnapshotRef` upon `dragStart` and added `planDropFromSnapshot` to strictly honor the exact drag payload regardless of React's batched update timing, bypassing `tree.state` during the native `drop` event while ensuring `tree.drop` correctly flushes `state.pending` to reset UI selection. Evidence: **TDD RED/GREEN PASS** (`DRAG-SET-2` and `FOLDER-NAME-1` added); suite passes 413/418 with 5 env skips; no other behavior changed. | `e041967c` correction | V4 UI implementation verified locally; Draft only; no final receipt | Exact-head guardrail, then keep isolated runtime available | Stop at Human Owner browser retest; no Production, Ready, receipt, or merge |
