...
- State: **V3 LOCAL ENGINEERING PASS / HUMAN OWNER RETEST PENDING (2026-09-22)** — the PostgreSQL 15 / TREE_V1 correction runtime now requalifies the Files-derived shared card shell, semantic selection bar, Vault toolbar/root copy, normal Files derivatives, encrypted Vault image/GIF/video posters, and client-only TREE_V1 full preview. The focused correction suite passes 406/411 with five declared environment-only skips, the production build passes, and the live local browser shows real image/GIF/video posters plus ready muted motion proxies. Upload, folder creation, marquee, move-in/move-out, TREE_V1 runtime, and the Vault tree route remain preserved. PR #171 stays Draft with no final receipt; Production remains untouched until the Human Owner retest.
