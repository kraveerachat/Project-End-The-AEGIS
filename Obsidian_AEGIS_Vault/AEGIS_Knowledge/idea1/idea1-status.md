...
- Proposed architecture: My Files and PC storage are separate namespaces; the On Your PC web location and Windows SMB mapping are two access paths to the same authoritative per-user filesystem tree, not synchronized copies. Initial visibility is refresh-based; a read-only web milestone is LIMITED, not full acceptance. Delete semantics, Linux-account/broker/deployment choices and architecture approval remain open. No implemented or deployed behavior is claimed.
...
| PCND-S2 | Draft design alignment 01: shared PC tree, storage-location semantics, refresh visibility, open deletion and future phase map | PASS | Resume at `bf504d4b8fd2867992bc68ae858b1cdb82d55a1f`; two Markdown paths only; existing research/security sections and unrelated canonical content preserved; vault PASS with 2 existing warnings; collaboration tests 24/24 PASS; Draft policy, diff/staged diff and targeted secret-pattern checks PASS | `469c79367eda68686e25465a62339cf7e5159fb3` (design/evidence checkpoint); this register update is a separate documentation checkpoint | Local planning alignment PASS only; no architecture acceptance or runtime claim | Publication/exact-head CI verification; owner review and all future implementation gates | Human architecture review after PR #150; no implementation |
