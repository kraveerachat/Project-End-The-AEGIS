...
- State: **V8 FRAME-SELECTION POLICY PASS / HUMAN OWNER NON-BLACK VIDEO RETEST PENDING (2026-09-23)** — the Vault poster path now mirrors Production Files' representative-frame policy client-side: `min(duration, clamp(duration*0.05, 0.5s, 3s))`, with zero reserved for invalid/non-positive duration or an explicit caller override. V7's stable scheduler and bounded `BLOB_NOT_READY` recovery remain intact. The required six-suite V8 matrix passes 74/74 with zero failures or skips; production build, governance, vault validation, and diff checks pass. No visual markup or styles changed, so Impeccable is not applicable. Pixel-level non-black correctness remains Human Browser gated because real video pixels are outside the automated mocks and the disposable acceptance password remains unavailable to the agent. The existing PostgreSQL 15 / TREE_V1 runtime is reused, Production remains untouched, and PR #171 stays Draft with no final receipt.
