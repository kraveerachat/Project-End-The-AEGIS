...
| VUX-S5 | Human-acceptance V4 corrections for Vault folder cards and multi-select drag pipeline | LOCAL PASS / OWNER RETEST PENDING | Root causes confirmed: `VaultFolderTile` rendered `childCount` natively unlike normal Files (violating the parity rule); the drag pipeline relied on a potentially stale React closure inside `VaultTreeScreen` causing N items to collapse to 1 during the drop action. Corrected `VaultFolderTile` to remove `childCount` and rely strictly on folder icon and name. Corrected `useVaultTree` to capture a synchronous `dragPayloadRef` upon `dragStart` and added `planDropFromSnapshot`. Further hardened the contract by allowing `intentOverride` inside `planDrop` so that the exact snapshot intent (and its exact `operationId`) is the ONLY authoritative intent passed to the reducer and `session.commit()`, ensuring strict one-to-one synchronization and clearing the UI selection perfectly. Replaced source-scan guards with pure behavioral reducer regression tests. Evidence: **TDD RED/GREEN PASS** (`DRAG-SET-1` to `5`, `MOVE-ATOMIC-1`, `MOVE-NOCOPY-1` explicitly tested in `vaultTreeReducer.test.js`); `vaultTreeReducer` 8/8 PASS, `vaultFilesUx` 7/7 PASS (source-scans removed), `vaultTreeOps` 9/9 PASS. Collaboration policy tests: 49/49 PASS. Build: PASS. | `9c171326` merge + test hardening | V4 UI implementation verified locally; exact head documented; Draft only; no final receipt | Keep isolated runtime available | Stop at Human Owner browser retest; no Production, Ready, receipt, or merge |
