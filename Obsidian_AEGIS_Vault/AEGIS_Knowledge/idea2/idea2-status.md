...
- On the local branch stacked over viewer-demand capture, YOLO+SFace recognition and the headless Docker fix, all 50 Detection Engine tests, all five PowerShell parses and the non-mutating installer `-WhatIf` preflight pass. **Automatic tunnel registration and reconnect after a fresh Windows sign-in remain unverified**, so this is still partial and must not be described as production-ready.
