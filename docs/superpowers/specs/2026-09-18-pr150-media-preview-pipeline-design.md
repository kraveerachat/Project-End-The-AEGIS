...
Boot order (`index.js`): parse `MediaLimits` → existing storage/bootstrap prerequisites → create the process runner → if `MEDIA_ENABLED` run `detectCapabilities()`, else use `CAPABILITIES_NONE` **without spawning anything** → `media/runtime.js` assembles the **real** service from limits + probed capabilities + trusted storage resolver + cache + queue + generators + evictor → `await service.init()` → `createApp({ env, mediaLimits, mediaService: service })` → `listen` → `service.start()` → on `SIGTERM`: `service.stop()` then normal server shutdown. The app never receives a stand-in service that is "upgraded later"; a test or a disabled deployment receives `disabledMediaService(limits, reason)` explicitly. `media/capabilities.js` runs `ffmpeg -version`, `ffmpeg -hide_banner -decoders`, `ffmpeg -hide_banner -encoders`, and `ffprobe -version` (each with a 10 s timeout), and loads sharp. It records a frozen capability object: `{ ffmpeg: { version, ok }, ffprobe: { ok }, encoders: { libx264, libwebp }, decoders: { gif, apng, webp, webpAnimated, av1, h264, vp8, vp9 }, sharp: { version, ok, avif } }`. Missing pieces **degrade** (`media-info` reports `UNSUPPORTED` with a `reason`) — they never prevent the Drive from starting, because file management must not depend on a thumbnail toolchain. `/healthz` gains a `media` block `{ enabled, reason, ffmpeg, sharp, cacheWritable, cacheVolume }` (additive; the existing `application/db/storage` probes are unchanged and health remains `ok` when only `media` is degraded). `cacheVolume ∈ { 'volume', 'ephemeral', 'unknown' }` is decided by `media/mountInfo.js`, which parses `/proc/self/mountinfo` for an exact mountpoint entry equal to the resolved `MEDIA_CACHE_DIR` (escaped-space aware); no entry but a writable directory → `ephemeral`; unreadable mountinfo or unsupported platform → `unknown`. Device-id comparison is not used. Production acceptance requires `cacheVolume = 'volume'`.
...
|---|---|---|
| TRANSIENT | timeout, `SIGKILL`ed, `ENOSPC` on cache, queue full, cache below free reserve | `MAX_TRANSIENT_RETRIES = 3`, `MAX_TOTAL_ATTEMPTS = 4`: attempt 1 fails → retry after 1 min; attempt 2 fails → retry after 5 min; attempt 3 fails → retry after 30 min; attempt 4 fails → `GENERATION_FAILED` with `reason`. No attempt starts before `nextRetryAt`. |
| PERMANENT | decoder error, corrupt container, family mismatch, pixel/duration guard, output too large, unsupported codec | no retry for this profile (`attempts = 1`); `GENERATION_FAILED` / `UNSUPPORTED` persisted in `state.json` |
...
The service receives the trusted storage resolver by injection (`createDerivativeService({ resolveStorageKey, keyExists, … })`, assembled by `media/runtime.js` from `storage/fileStore.js`'s `resolveKey`/`keyExists`; tests inject a temp-root resolver and never depend on the process-global `STORAGE_ROOT`). Every generator receives `absPath = resolveStorageKey(row.path)` and nothing else path-like: a key that does not resolve under the storage root or does not exist yields `UNSUPPORTED / SOURCE_MISSING` and no generator is called. Output paths are computed from `sha256` (validated `^[0-9a-f]{64}$`) and the cache root only. No string from the request, the filename, or the DB ever reaches a command line unvalidated; there is no shell.
...
|---|---|---|
| FFmpeg/ffprobe missing at boot | Drive starts; `media.enabled=false`, `reason=TOOLS_MISSING`; all `media-info` → `200 { status: 'UNSUPPORTED', reason: 'MEDIA_DISABLED' }`; poster/motion → `415 { code: 'MEDIA_DISABLED' }`; `scheduleForFile` returns `false`; queue never started; `/healthz.media` shows it | icons with type badges; Preview dialog still works from originals |
| `MEDIA_ENABLED=false` (kill switch) | identical wire semantics to the row above with `reason=MEDIA_DISABLED`; additionally **no** `ffmpeg`/`ffprobe` is spawned and no sharp work runs at boot or later; Files list/rename/move/upload/download unaffected; `/healthz` healthy | as above |
...
- bounded read, per container class: sparse fixtures are **structurally constructed**, not merely truncated — a box/element rewriter inserts a sparse `free` box (MP4) or `Void` element (WebM) of 10 GiB (20 GiB in the Linux design gate) inside a valid short file and re-bases every offset (`stco`→`co64`, `SeekHead`, `CuePosition`) so that (a) faststart MP4 keeps `moov` before `mdat`, (b) moov-at-end MP4 keeps `moov` **in the final tail of the final file**, (c) WebM-with-Cues keeps a real `Cues` element, (d) WebM-without-Cues has none. A fixture-integrity phase (box/element map with offsets + ffprobe recognition) runs before any performance assertion; a class whose sparse representation cannot be built and verified is recorded `NOT_PROVEN` and its performance rows are skipped — never passed on trailing zero padding. For proven classes the test records `rchar`/`read_bytes` of the ffprobe/ffmpeg children and asserts poster + motion succeed **or** fail truthfully within timeout; the `< 64 MiB` bound is asserted only for classes (a) and (c), and the measured figure for (b) and (d) is recorded as evidence.
- byte-size classes are proven by `stat`, not by dimensions: `200_MB_CLASS` = 180–240 MiB on disk, `300_MB_CLASS` = 280–340 MiB, `GIF_49MB_CLASS` = 45–55 MiB; generators use valid, deterministic, incompressible pixel data (16-bit / RGBA noise) and adjust bit depth, channels or frame count until the encoded original is in band, printing `SOURCE_BYTES / WIDTH / HEIGHT / PIXELS / FORMAT`; a fixture that is expected to succeed also asserts `PIXELS ≤ MEDIA_MAX_SOURCE_PIXELS`, and the over-pixel case is a separate `DIMENSIONS` rejection test.
- toolchain provenance: the authoritative large-media and responsiveness evidence is produced **inside a container created from the packaged candidate image** (`MEDIA_TOOLCHAIN_PROVENANCE=PACKAGED_CANDIDATE`, with `CANDIDATE_IMAGE_ID / FFMPEG_VERSION / FFPROBE_VERSION / SHARP_VERSION / ALPINE_VERSION` recorded); a run with a host FFmpeg is `HOST_TOOLCHAIN_RESULT=SUPPLEMENTAL` and never substitutes for the candidate-tool PASS.
...
- large still: a 7,000 × 5,000 16-bit RGB noise PNG (35 MP, ≈ 210 MiB on disk → `200_MB_CLASS` by `stat`) and an 8,000 × 5,000 16-bit RGBA noise PNG (40 MP, ≈ 320 MiB → `300_MB_CLASS`) generated at test time → poster succeeds; peak RSS of the sharp path recorded; a 9,000 × 5,000 (45 MP, over the guard) → `UNSUPPORTED / DIMENSIONS` without decoding (elapsed < 200 ms).
...
- Final main freeze: immediately before Ready/merge, `FINAL_MAIN_SHA = origin/main` and `FINAL_BRANCH_HEAD = origin/feat/idea1-files-management-ux` are recorded; if `main` advanced after the last reconciliation, Ready/merge STOPS, `main` is merged normally, the IDEA1/media tree impact is proven, the exact-SHA verification is re-run on the reconciled head, and Production evidence is reassessed when the source/runtime tree changed (docs-only/non-IDEA1 drift is recorded with a tree-equivalence proof). No Ready/merge against a stale `main`.
